(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../util/number"], factory);
    }
    ...
    function convertMeta(id) {
        var v;
        for (v in meta[id]) {
            var arr = meta[id][v].split(',');
            arr.map(function (l) { meta[id][l] = v; });
        }
    }
    var conversions = ['type', 'scope', 'likelyScript'];
    conversions.map(convertMeta);
    function getMeta(o) {
        if (o === void 0) { o = { iso1: 'en' }; }
        var id = o.iso1 || o.iso2 || o.iso3;
        var rank = meta.rank.indexOf(id);
        var low = (meta.speakersWorldPopulation / 0.99) / 1000;
        o.id = id;
        if (!o.nameT && !!o.name) {
            o.nameT = o.name;
        }
        o.names = (!o.names) ? [] : o.names.split(',');
        o.names.unshift(o.name);
        o.names.unshift(o.nameT);
        o.names.forEach(function (_n) { (_n.indexOf('’') > -1) && o.names.push(_n.replace(/’/g, "'")); });
        var likely = { Latn: [] };
        var _o = {}, k;
        ...
        for (k in likely) {
            var country = (meta[(k === 'Latn') ? 'likelyCountryLatin' : 'likelyCountry'][id]) || 'ZZ';
            if (likely[k].indexOf(country) < 0) {
                likely[k].push(country);
            }
        }
        o.locales = likely;
        ...
        if (rank > -1) {
            var count = ((rank < meta.speakers.length) ? meta.speakers[rank] : low);
            o.speakers.percent = number_1.preciseRound(count / meta.speakersWorldPopulation * 100);
