...
export function formatDNodes(nodes, depth = 0) {
    ...
    let formattedNode = nodes.reduce((result, node, index) => {
        if (!node || node === true) {
            return result;
        }
...
function decorate(actual, expected) {
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (expectedNode && expectedNode.widgetConstructor === Ignore) {
...
export function assertRender(actual, expected, message) {
    ...
    let diffFound = false;
    const parsedDiff = diffResult.reduce((result, part, index) => {
        if (part.added) {
            diffFound = true;
...
//# sourceMappingURL=assertRender.mjs.map