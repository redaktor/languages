...
const meta: any[] = [];
var iso: string, o: any;
for (iso in metaRaw) {
  o = {name: metaRaw[iso][0]};
  o[(iso.length === 4) ? 'iso' : 'id'] = iso;
  if (metaRaw[iso].length > 1) {
    const ki = (!!metaRaw[iso][1]) ? ['detect',1] : ['subdetect',2];
    o[ki[0]] = metaRaw[iso][ki[1]];
  }
  meta.push(o);
}
export default meta;
